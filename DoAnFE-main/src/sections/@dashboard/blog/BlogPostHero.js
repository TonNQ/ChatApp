...
import { Box, Avatar, SpeedDial, Typography, SpeedDialAction, Hidden } from '@mui/material';
// hooks
...
export default function BlogPostHero({ post }) {
  const { cover, title, user, createdAt } = post;
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      <OverlayStyle />
      <Image alt="post cover" src={`${Config.BACKEND_URL}${cover}`} ratio="16/9" />
    </Box>
  );
}
